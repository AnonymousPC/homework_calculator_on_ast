    ...
    requires ( matrix.dimension() == 2 ) and ( number_type<matrix_value_type> or complex_type<matrix_value_type> )
{
    if constexpr ( int_type<matrix_value_type> or complex_int_type<matrix_value_type> )
        ...
        {
            throw math_error("cannot calculate singular of matrix: [[from {}]] {}", "eigen", e.what());
        }
}
    ...
    requires ( matrix.dimension() == 2 ) and ( number_type<matrix_value_type> or complex_type<matrix_value_type> )
{
    if constexpr ( int_type<matrix_value_type> or complex_int_type<matrix_value_type> )
        ...
        {
            throw math_error("cannot calculate singular_value of matrix: [[from {}]] {}", "eigen", e.what());
        }
}
    ...
    requires ( matrix.dimension() == 2 ) and ( number_type<matrix_value_type> or complex_type<matrix_value_type> )
{
    if constexpr ( int_type<matrix_value_type> or complex_int_type<matrix_value_type> )
        ...
        {
            throw math_error("cannot calculate singular_vector of matrix: [[from {}]] {}", "eigen", e.what());
        }
}
