...
namespace ap
{
    ...
    template < class type > concept fraction_type = requires { typename type::fraction_tag; };
    template < class type > concept angle_type    = requires { typename type::angle_tag;    };
    template < class type > concept degree_type   = requires { typename type::degree_tag;   };
    template < class type > concept radian_type   = requires { typename type::radian_tag;   };
    ...
    template < class type > concept complex_int_type = complex_type<type> and []
    {
    ...
    template < class type > concept complex_float_type = complex_type<type> and []
    {
        static_assert ( requires { typename type::value_type; }, "class provides complex_tag but not provides value_type" );
        if constexpr ( requires { typename type::value_type; } )
    ...
    constexpr number_type    auto  cross           ( const array_type auto&  left, const array_type auto&  right ) requires ( left  .dimension() ==  1 ) and   number_type<left_value_type>                                        and ( right.dimension() ==  1 ) and   number_type<right_value_type>; // Here complex-version is undefined.
