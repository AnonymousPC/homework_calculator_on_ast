...
constexpr binary_search_tree<type,compares>::const_iterator binary_search_tree<type,compares>::locate ( const auto& val ) const
    requires requires { compares::equal_to(val,std::declval<type>()); }
{
    ...
    else // size() >= 1
    {
        ...
        while ( true )
        {
            if ( compares::equal_to ( val, static_cast<const type&>(*ptr) ) )    // ==
                return ptr;

            if ( compares::less ( val, static_cast<const type&>(*ptr) ) )        // <
                if ( ptr->has_left() )
                    ptr = &( ptr->left() );
                else
                    return ptr;
...
template < class type, class compares >
constexpr binary_search_tree<type,compares>::const_iterator binary_search_tree<type,compares>::push ( auto&& val, const_iterator it )
...
template < class type, class compares >
constexpr binary_search_tree<type,compares>::const_iterator binary_search_tree<type,compares>::pop ( const auto& val, const_iterator it )
    requires requires { compares::equal_to(val,std::declval<type>()); }
{
    #if debug
        if ( size() == 0 )
    ...
    else // size() >= 2
    {
        ...
        #if debug
            if ( not compares::equal_to ( val, static_cast<const type&>(*ptr) ) )
...
template < class type, class compares >
constexpr binary_search_tree<type,compares>::const_iterator binary_search_tree<type,compares>::pop_leaf ( const_iterator it )
{
...
template < class type, class compares >
constexpr binary_search_tree<type,compares>::const_iterator binary_search_tree<type,compares>::pop_branch ( const_iterator it )
{
    node& curr = *const_cast<node*> ( static_cast<const node*> ( it ) );

    const_iterator high = curr.is_left() ? &( curr.cut_up().link_left ( curr.has_left() ? curr.cut_left() otherwise curr.cut_right() ) ) otherwise
...
template < class type, class compares >
constexpr binary_search_tree<type,compares>::const_iterator binary_search_tree<type,compares>::pop_crotch ( const_iterator it )
{
    node& curr = *const_cast<node*> ( static_cast<const node*> ( it ) );
    ...
    if ( &curr.right() != &next ) // Not adjacent
    {
        ...
        node* next_right_ptr = next.has_right() ? &next.cut_right() otherwise nullptr;

        curr_is_left ? curr_up.link_left ( next ) otherwise curr_up.link_right ( next );
        next.link_left  ( curr_left  );
        next.link_right ( curr_right );

        next_is_left ? next_up.link_left ( curr ) otherwise next_up.link_right ( curr );
        if ( next_right_ptr != nullptr )
            curr.link_right ( *next_right_ptr );
    }
