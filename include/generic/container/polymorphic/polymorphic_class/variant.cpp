...
constexpr variant<types...>::variant ( )
    extends data ( first_type_of<types...>() ),
            flag ( 1 )
{
...
{
    self.data.assign ( right, self.flag );
...
constexpr variant<types...>::operator result_type& ( )
    requires aux::same_as_only_one_of<result_type,types...>
{
    if ( type() != aux::index_of_unique_same_type<result_type,types...> )
        throw type_error("bad variant access: cannot get {} from {} (in which type == {}, index == {})",
                         typeid(result_type), typeid(self), type() == 0 ? string(typeid(void)) otherwise string(*array{&(typeid(types))...}[self.type()]), self.type());

    return data.template get<result_type>();
}

template < class... types >
template < class result_type >
constexpr variant<types...>::operator const result_type& ( ) const
    requires aux::same_as_only_one_of<result_type,types...>
{
    if ( type() != aux::index_of_unique_same_type<result_type,types...> )
        throw type_error("bad variant access: cannot get {} from {} (in which type == {}, index == {})",
...
template < class... types >
constexpr int variant<types...>::type ( ) const
{
    return flag;
}
...
{
    return data.visit ( std::move(visitor), self.flag );
}
...
/* In variant::union_obj, the flag and data are controlled together.
 * data.clear() must accept a pos-arg, and std::move() will never clear
 * the storage of the right one.
 */
